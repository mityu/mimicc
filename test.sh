...
assert 24 'int x[2][3]; int main() { return sizeof(x);}'
assert 12 'int x[2][3]; int main() { return sizeof(x[0]);}'
assert 16 'int x[2][3][4]; int main() { return sizeof(x[0][0]);}'
...
assert 15 'int *p, a[2], n; int main() {a[0] = 3; a[1] = 7; n = 1; p = &n; *p = 8; return a[1] + n;}'
assert 105 'int a[3]; int b; int main() {a[0] = 3; a[1] = 5; a[2] = 7; return a[0] * a[1] * a[2];}'
assert 3 'int x[2][3]; int main() {int i, j; for (i=0; i<2; ++i) for (j=0; j<3; ++j) x[i][j] = 2 + i + j; return x[0][1];}'
